...
export const AppProvider = ({ children }) => {
  ...
  const syncToSupabase = useCallback(async (currentBills) => {
    ...
    // Handle deletions (sync local state with remote)
  ...
  }, [bills, isDataLoading, storageKey, syncToSupabase]);
  ...
  const saveBill = useCallback(async (form, isEditing) => {
    ...
    if (isEditing) {
      ...
      newBills = [...bills, { ...form, id: crypto.randomUUID(), amount: Number(form.amount), paid: false }];
    }
    setBills(newBills);
    localStorage.setItem(storageKey(), JSON.stringify(newBills));
  ...
  const deleteService = useCallback(async (serviceName) => {
    const newBills = bills.filter(b => b.name !== serviceName || b.paid === true);
    setBills(newBills);
    localStorage.setItem(storageKey(), JSON.stringify(newBills));
    await syncToSupabase(newBills);
  }, [bills, storageKey, syncToSupabase]);
  ...
  const value = useMemo(() => ({
    ...
    saveBill, deleteService, togglePaid, signOut, houseId
  }), [user, bills, isDataLoading, autoGeneratedCount, saveBill, deleteService, togglePaid, signOut, houseId]);
