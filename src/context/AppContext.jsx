...
export const AppProvider = ({ children }) => {
  ...
  // Categorías personalizadas sincronizadas en la nube y localStorage
  const [customCategories, setCustomCategories] = useState(() => {
    try {
      const raw = localStorage.getItem('home_custom_categories');
  ...
  useEffect(() => {
    ...
    const finishAuthCheck = (session) => {
      ...
      if (session) {
        ...
        // Sincronizar categorías personalizadas desde la nube de Supabase (multi-dispositivo)
        ...
        if (Array.isArray(cloudCategories) && cloudCategories.length > 0) {
          ...
          try {
            localStorage.setItem('home_custom_categories', JSON.stringify(cloudCategories));
          } catch {}
        }
      }
  ...
  const saveCustomCategories = useCallback(async (newCategories) => {
    ...
    try {
      localStorage.setItem('home_custom_categories', JSON.stringify(newCategories));
    } catch {}

    // Sincronizar en la nube en user_metadata para que esté disponible en cualquier dispositivo
    if (user) {
      try {
